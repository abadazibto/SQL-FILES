...
SELECT P.PRODUCT_REF, P.PRODUCT_NAME, P.QUANTITY, P.SUPPLIER_NUMBER, P.UNIT_PRICE, P.UNITS_ON_ORDER
FROM PRODUCTS P
WHERE P.QUANTITY LIKE '%bottle(s)%'
	 AND P.PRODUCT_NAME LIKE '__t%' OR P.PRODUCT_NAME LIKE '__T%'
...
FROM CUSTOMERS C
JOIN SUPPLIERS S
    ON C.POSTAL_CODE = S.POSTAL_CODE
WHERE C.ADDRESS IN (SELECT S.CITY + ', ' + S.POSTAL_CODE + ', ' + S.COUNTRY
                    FROM SUPPLIERS S
                    WHERE S.POSTAL_CODE LIKE '%4SD'
                      AND S.SUPPLIER_NUMBER = 1);
...
*/
SELECT ORDER_NUMBER, SUM(UNIT_PRICE * QUANTITY) AS "TOTAL ORDER AMOUNT BEFORE DISCOUNT",
	 CASE 
...
WHERE CA.CATEGORY_NAME = 'DESSERT' OR CA.CATEGORY_NAME IS NULL
AND C.CITY = 'Berlin'
GROUP BY C.CUSTOMER_CODE
HAVING COUNT (CASE WHEN CA.CATEGORY_NAME = 'DESSERT' THEN 1 ELSE NULL END) <= 1;
...
HAVING COUNT(DISTINCT OD.PRODUCT_REF) = (SELECT COUNT(P.PRODUCT_REF) 
											FROM PRODUCTS P);
...
SELECT C.CUSTOMER_CODE, COUNT(OD.QUANTITY) AS "NUMBER OF ORDERS"
FROM CUSTOMERS C
LEFT JOIN ORDERS O
ON C.CUSTOMER_CODE = O.CUSTOMER_CODE
LEFT JOIN ORDER_DETAILS OD
...
/* Display the number of orders placed in 1996, the number of orders placed in 1997, and the difference between these two numbers. The resulting table should display
...
ABS (SUM(CASE WHEN YEAR(ORDER_DATE) = 1997 THEN 1 ELSE 0 END) - SUM(CASE WHEN YEAR(ORDER_DATE) = 1996 THEN 1 ELSE 0 END))
 AS Difference
FROM ORDERS;
